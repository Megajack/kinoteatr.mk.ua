...
/**
 * @file
 * Contains theme override functions and process & preprocess functions for kinoteatr
 */

// Auto-rebuild the theme registry during theme development.
if (theme_get_setting('kinoteatr_clear_registry')) {
  // Rebuild .info data.
 ...
 */
function kinoteatr_html_head_alter(&$head_elements) {
 ...
 */
function kinoteatr_preprocess_search_block_form(&$vars) {
 ...
 */
function kinoteatr_preprocess(&$vars, $hook) {
  $vars['kinoteatr_path'] = base_path() . path_to_theme();
}

/**
 * Implements template_preprocess_html().
 */
function kinoteatr_preprocess_html(&$vars) {
  
  $vars['doctype'] = _kinoteatr_doctype();
  $vars['rdf'] = _kinoteatr_rdf($vars);
  ...
  if (!$vars['is_front']) {
    ...
    if (isset($page_name)) {
      $vars['classes_array'][] = kinoteatr_id_safe('page-' . $page_name);
    }

    $vars['classes_array'][] = kinoteatr_id_safe('section-' . $section);
 ...
 */
function kinoteatr_preprocess_page(&$vars) {
 ...
 */
function kinoteatr_preprocess_maintenance_page(&$vars) {
  ...
  drupal_add_css(drupal_get_path('theme', 'kinoteatr') . '/css/maintenance-page.css');
}
 ...
 */
function kinoteatr_preprocess_node(&$vars) {
  // Striping class
 ...
 */
function kinoteatr_preprocess_block(&$vars, $hook) {
 ...
 */
function kinoteatr_menu_tree($vars) {
 ...
 */
function kinoteatr_field__taxonomy_term_reference($vars) {
  $output = '';
 ...
 */
function kinoteatr_breadcrumb($vars) {
  
  $breadcrumb = isset($vars['breadcrumb']) ? $vars['breadcrumb'] : array();

// Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('kinoteatr_breadcrumb_display');
  if ($show_breadcrumb == 'yes') {
  
	  if (theme_get_setting('kinoteatr_breadcrumb_hideonlyfront')) {
		$condition = count($breadcrumb) > 1;
	  } else {
		$condition = !empty($breadcrumb);
	  }
	  
	  if(theme_get_setting('kinoteatr_breadcrumb_showtitle')) {
	  ...
	  $separator = theme_get_setting('kinoteatr_breadcrumb_separator');
 ...
 * @return bool
 */
function kinoteatr_tabs_float() {
  $float = (bool) theme_get_setting('kinoteatr_tabs_float');
  $float_node = (bool) theme_get_setting('kinoteatr_tabs_node');
...
function kinoteatr_id_safe($string) {
  // Strip accents
 ...
 * @ingroup themeable
 */
function kinoteatr_menu_link(array $vars) {
  ...
  $element['#attributes']['class'][] = kinoteatr_id_safe($element['#title']);
 ...
 */
function kinoteatr_preprocess_menu_local_task(&$vars) {
 ...
 */
function kinoteatr_menu_local_tasks(&$vars) {
  $output = '';
